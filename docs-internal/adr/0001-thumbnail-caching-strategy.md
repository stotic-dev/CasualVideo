...
- ステータス: 採用
- 日付: 2026-05-30
- 関連: F-1（写真ライブラリの動画一覧）, `docs-internal/architecture.md`, `docs-internal/requirements.md`（非機能要件: パフォーマンス）
...
- キャッシュは `Infra` 内のグローバルに近い状態となる。テスト対象は Core / Features のロジック（`docs-internal/testing.md`）であり `Infra` は対象外のため、テスタビリティ上の問題は生じない。
